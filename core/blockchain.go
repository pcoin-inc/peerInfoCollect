...
import (
	...
	"peerInfoCollect/internal/syncx"
	"peerInfoCollect/log"
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	for i, block := range chain {
		...
		seals[i] = verifySeals
		log.Info("insert chain---","num",headers[i].Number.Uint64())
	}
	...
	//block, err := it.next()
	//if err != nil {
	//	log.Error("insertChain err","err",err.Error())
	//	return 0, err
	//}


	block := chain[0]
	log.Info("insert chain info","block num",block.NumberU64(),"block hash",block.Hash().String())

	rawdb.WriteBlock(bc.db,block)
	rawdb.WriteTd(bc.db,block.Hash(),block.NumberU64(),block.Header().Difficulty)
	bc.currentBlock.Store(block)
	/**
