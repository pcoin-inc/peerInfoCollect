...
import (
	...
	"peerInfoCollect/consensus/misc"
...
func (beacon *Beacon) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
	...
	if parent == nil {
		log.Info("VerifyHeader---beacon--","num",header.Number.Uint64(),"parent",header.Number.Uint64()-1)
		...
		for i, header := range headers {
			...
			if parent == nil {
				...
				case results <- consensus.ErrUnknownAncestor:
					log.Info("verifyHeaders---beacon---1--","err",consensus.ErrUnknownAncestor.Error())
				}
				continue
			}
