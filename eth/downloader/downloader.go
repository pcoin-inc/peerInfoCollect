...
import (
	...
	"peerInfoCollect/core/state/snapshot"
	"peerInfoCollect/core/types"
	"peerInfoCollect/eth/protocols/snap"
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	log.Info("syncWithPeer----","hash",hash.String(),"td",td.Uint64())
	...
	if !beaconMode {
		...
		if latest != nil {
			log.Info("syncWithPeer---","latest num",latest.Number.Uint64())
		}
	} else {
	...
	log.Info("latest num info","height",height)
	...
	log.Info("findAncestor---","origin num",origin)

	//TODO 现在origin已经手动写了一个比远端的高度少一个
	...
	if d.syncInitHook != nil {
		log.Info("syncInitHook---","走了origin",origin)
...
func (d *Downloader) fetchHead(p *peerConnection) (head *types.Header, pivot *types.Header, err error) {
	...
	latest, num := p.peer.Head()
	fetch := 1
	if mode == SnapSync {
		fetch = 2 // head + pivot headers
	}
	log.Info("fetchHead---info","peer head hash",latest.String(),"peer td",num.Uint64())
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header) (uint64, error) {
	...
	log.Info("findAncestor----call","remoteHeight",remoteHeight,"localHeight",localHeight)

	ancestor := localHeight-1
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (uint64, error) {
	...
	////modify by echo should find where the response is
	//err = d.fetchBodies(14479906,false)
	//if err != nil {
	//	log.Error("findAncestorSpanSearch--err","err",err.Error())
	//}

	//其实也不用特意去下载区块体直接返回一个比远端给的区块头小一个号的就行
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64, head uint64) error {
	...
	for {
		...
		case skeleton:
			//TODO 这里从共同祖先开始下载了191个区块头 这地方需要改一下 问题是哪来的那么多区块头
			...
			headers, hashes, err = d.fetchHeadersByNumber(p, from+uint64(1), 1, 1, false)
	...
	for {
		...
		case task := <-d.headerProcCh:
			...
			for _,v := range headers {
				log.Info("process header --called","header num",v.Number.Uint64(),"header hash",v.Hash().String())
			}
